...
def decode_bbox(pred_hms, pred_whs, pred_offsets, confidence, cuda):
    ...
    for batch in range(b):
        ...
        confs = torch.chunk(heat_map, 4, dim=-1)
...
def postprocess2(prediction, need_nms, image_shape, input_shape, letterbox_image, nms_thres=0.4):
    output = [None for _ in range(len(prediction))]
