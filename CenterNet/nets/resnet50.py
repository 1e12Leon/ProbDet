...
def resnet50(pretrained = True):
    ...
    return features
def yolov7(pretrained = True):
    phi = 'l'
    transition_channels = {'l': 32, 'x': 40}[phi]
    block_channels = 32
    n = {'l': 4, 'x': 6}[phi]
    model = yolo_Backbone(transition_channels, block_channels, n, phi, pretrained=False)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['yolov7'], model_dir = 'model_data/')
        model.load_state_dict(state_dict)
    #----------------------------------------------------------#
    #   获取特征提取部分
    #----------------------------------------------------------#
    features = list([model.stem, model.dark2, model.dark3, model.dark4, model.dark5])
    features = nn.Sequential(*features)
    return features
