...
from yolov7.utils.utils_map import get_coco_map, get_map
...
if __name__ == "__main__":
    ...
    # --------------------------------------------------------------------------------------#
    confidence_yolo = 0.001
    confidence_centernet = 0.04
    # --------------------------------------------------------------------------------------#
    ...
    #   默认指向根目录下的VOC数据集
    # -------------------------------------------------------#
    VOCdevkit_path = r'D:\Deep_Learning_folds\ProbEn\yolov7\VOCdevkit'
    # -------------------------------------------------------#
    #   结果输出的文件夹，默认为map_out
    # -------------------------------------------------------#
    ...
    if map_mode == 0 or map_mode == 1:
        print("Load model.")
        yolo = YOLO(confidence=confidence_yolo, nms_iou=nms_iou)
        centernet = CenterNet(confidence=confidence_centernet, nms_iou=nms_iou)
        proben = ProbEn()
        ...
        for image_id in tqdm(image_ids):
            ...
            proben.get_map_txt(image_id, class_names, map_out_path, dets_yolo, scores_yolo, dets_centernet, scores_centernet)
