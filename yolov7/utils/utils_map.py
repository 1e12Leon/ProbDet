...
def get_map(MINOVERLAP, draw_plot, score_threhold=0.5, path = './map_out'):
    ...
    for class_index, class_name in enumerate(gt_classes):
        bounding_boxes = []
        for txt_file in dr_files_list:  #   此循环结束得到按置信度降序排序的检测结果（每个结果一行，形式为：confidence,file_id,bbox）
            file_id = txt_file.split(".txt",1)[0]
            file_id = os.path.basename(os.path.normpath(file_id))
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            #   对包含所有检测结果的dr_data文件中的每一条检测结果detection
            for idx, detection in enumerate(dr_data):
                file_id     = detection["file_id"]
                score[idx]  = float(detection["confidence"])
                if score[idx] >= score_threhold:
                    score_threhold_idx = idx    #找到之后一个分数满足阀值的id，方便计算后续的Recall和Precision值
                ...
                #   找到这一条检测结果对应的file_id的真实标注框的json文件，此json文件含有一个或多个真实框信息
                ...
                for obj in ground_truth_data:
                    if obj["class_name"] == class_name:
                        ...
                        if iw > 0 and ih > 0:   #   计算IOU
                            ...
                            ov = iw * ih / ua
                            if ov > ovmax:  #   拿到最大的IOU(ovmax)以及此时所对应的真实框信息(obj、gt_match)
                                ovmax = ov
                                gt_match = obj
                ...
                #   最大IOU大于min_overlap的话
                #   拿到tp、fp数组和count_true_positives[class_name]值
            ...
            #   在result/中各自的文件夹下画AP、F1、Precison、Recall的各类别的图像
            if draw_plot:
                plt.plot(rec, prec, '-o')
