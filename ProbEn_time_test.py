...
if __name__ == '__main__':
    ...
    for index in matches:
        ...
        draw.rectangle([left + thickness, top + thickness, right - thickness, bottom - thickness], outline='green',
                       width=2)
    ...
    while(interval < time_interval):
        ...
        try:
            ...
            dets_centernet = np.asarray(dets_centernet)
            scores_centernet = np.asarray(scores_centernet)
            t_centernet = time.time() - t2
            print("centernet时间:", t_centernet)
